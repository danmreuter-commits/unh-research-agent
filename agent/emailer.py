...
Your sole job: compress competitive intelligence and regulatory findings into a ≤100-word \
bullet-point briefing for a UnitedHealth Group (UNH) investor/executive.
...
3. Each bullet: company or agency name (bold in HTML) + 5–8 words of signal.
4. Lead with the most strategically significant finding (CMS regulatory changes first).
5. Omit low-signal noise. If a finding doesn't matter to a UNH investor, skip it.
...
7. If no meaningful findings exist, output exactly one line:
   • No significant competitive or regulatory updates today.
"""
...
def _format_findings_block(findings: list[dict]) -> str:
    ...
    lines = []
    for f in findings:
        src = f"[{f['vc_firm']}] " if f.get("vc_firm") and f["vc_firm"] != "N/A" else ""
        lines.append(f"- [{f['relevance']}] {f['company']} ({f['type']}): {src}{f['description']}")
    return "\n".join(lines)
...
<body style="font-family:Arial,Helvetica,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#1a1a2e;">
  <h2 style="font-size:18px;border-bottom:2px solid #002677;padding-bottom:8px;margin-bottom:16px;">
    UNH Intel Brief &mdash; {today_str}
  </h2>
  ...
  <p style="font-size:11px;color:#999;margin-top:8px;">
    UNH Research Agent &bull; Automated daily competitive &amp; regulatory brief &bull; {today_str}
  </p>
</body>
</html>"""
...
def send_digest(findings: list[dict], digest_text: str | None = None) -> None:
    today_str = datetime.now().strftime("%B %d, %Y")
    subject = f"UNH Intel Brief – {today_str}"
